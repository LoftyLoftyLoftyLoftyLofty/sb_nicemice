...
function build(directory, config, parameters, level, seed)
  ...
  if builderConfig.animationParts then
    ...
    for k, v in pairs(builderConfig.animationParts) do
      if type(v) == "table" then
        ...
        config.fullbrightParts[k] = util.absolutePath(directory, string.gsub(v.path, "<variant>", (parameters.animationPartVariants[k] or "").."fullbright"))
  ...
  if builderConfig.gunParts then
    ...
    for _,part in ipairs(builderConfig.gunParts) do
      ...
      local partOffset = {config.baseOffset[1] + imageOffset[1] / 8, config.baseOffset[2]}
      config.animationCustom.animatedParts.parts[part].properties.offset = partOffset
      partImagePositions[part] = copy(imageOffset)
      imageOffset = vec2.add(imageOffset, {imageSize[1] / 2, 0})

      -- add a matching fullbright overlay part, if one was generated for this part
      if config.fullbrightParts and config.fullbrightParts[part] then
        local fullbrightPartName = part .. "fullbright"
        construct(config.animationCustom.animatedParts.parts, fullbrightPartName, "properties")
        config.animationParts[fullbrightPartName] = config.fullbrightParts[part]
        config.animationCustom.animatedParts.parts[fullbrightPartName].properties.offset = partOffset
        config.animationCustom.animatedParts.parts[fullbrightPartName].properties.fullbright = true
        config.animationCustom.animatedParts.parts[fullbrightPartName].properties.zLevel = 1
      end
    end
  ...
  if not config.inventoryIcon and config.animationParts then
    ...
    for _,partName in pairs(parts) do
      ...
      -- add matching fullbright overlay, if one exists for this part
      if config.fullbrightParts and config.fullbrightParts[partName] then
        local fullbrightDrawable = {
          image = config.fullbrightParts[partName] .. config.paletteSwaps,
          position = partImagePositions[partName]
        }
        table.insert(config.inventoryIcon, fullbrightDrawable)
      end
    end
  end
